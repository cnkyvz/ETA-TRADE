...
RedirectableRequest.prototype.write = function (data, encoding, callback) {
  ...
  if (!isString$4(data) && !isBuffer$2(data)) {
...
function isSubdomain(subdomain, domain) {
  assert(isString$4(subdomain) && isString$4(domain));
...
const readBlob = /*#__PURE__*/function () {
  var _ref = _wrapAsyncGenerator(function* (blob) {
    if (blob.stream) {
      yield* _asyncGeneratorDelegate(_asyncIterator(blob.stream()));
    } else if (blob.arrayBuffer) {
      yield yield _awaitAsyncGenerator(blob.arrayBuffer());
    } else if (blob[asyncIterator]) {
      yield* _asyncGeneratorDelegate(_asyncIterator(blob[asyncIterator]()));
    } else {
      yield blob;
    }
  });
...
const formDataToStream = (form, headersHandler, options) => {
  ...
  return stream.Readable.from(_wrapAsyncGenerator(function* () {
    for (const part of parts) {
      yield boundaryBytes;
      yield* _asyncGeneratorDelegate(_asyncIterator(part.encode()));
    }
    yield footerBytes;
  })());
};
...
const buildAddressEntry = (address, family) => resolveFamily(utils$1$1.isObject(address) ? address : {
  address,
  family
});
...
const readStream = /*#__PURE__*/function () {
  var _ref4 = _wrapAsyncGenerator(function* (stream) {
...
isFetchSupported && (res => {
  ['text', 'arrayBuffer', 'blob', 'formData', 'stream'].forEach(type => {
    !resolvers[type] && (resolvers[type] = utils$1$1.isFunction(res[type]) ? res => res[type]() : (_, config) => {
...
function createClient(params) {
  ...
  const http = createHttpClient(axios$1, config);
